...
export async function handlePowerSetupSelect(
  ...
  const setup = await gameService.getMyPowerSetup(
    parsed.gameId,
    interaction.user.id,
  );

  if (setup === undefined) {
    throw new Error("This player has no power setup.");
  }

  const selectedTargets = interaction.values.map((playerId) => {
    const target = setup.targets.find(
      (candidate) => candidate.playerId === playerId,
    );

    if (target === undefined) {
      throw new Error(`Unknown selected power target: ${playerId}`);
    }

    return target;
  });

  const game = await gameService.configurePowerSetup(
    parsed.gameId,
    interaction.user.id,
    interaction.values,
  );

  if (setup.powerCode === "lovers-bond") {
    await notifyLovers(interaction, selectedTargets);
  }
...
async function notifyLovers(
  interaction: StringSelectMenuInteraction,
  targets: readonly LoverTarget[],
): Promise<void> {
  for (const target of targets) {
    const partners = targets.filter(
      (candidate) => candidate.playerId !== target.playerId,
    );

    const partnerMentions = partners
