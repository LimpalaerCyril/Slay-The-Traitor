...
export async function handleRoleSetupButton(
  ...
  if (parsed === undefined || parsed.scope !== "setup") {
    return false;
  }

  switch (parsed.action) {
    case "open":
      await handleOpen(interaction, gameService, parsed.gameId);
...
async function handleOpen(
  ...
  const setup = await gameService.getMyRoleSetup(gameId, interaction.user.id);
  ...
  await interaction.reply({
    content: createRoleSetupVariantContent(setup),

    components: createRoleSetupVariantControls(gameId, setup),
...
async function handleVariant(
  ...
  if (setup.setupCompleted) {
    await interaction.editReply({
      content: "✅ Votre rôle est déjà configuré.",
  ...
  const variant = setup.variants.find(
    (candidate) => candidate.code === variantCode,
  );

  if (variant === undefined) {
    throw new Error(`Unknown role variant: ${variantCode}`);
  }
  ...
  if (targetSelection === undefined) {
    ...
    await interaction.editReply({
      content: "✅ Votre rôle est configuré.",
  ...
  const eligibleTargets = setup.targets.filter(
    (target) =>
      targetSelection.allowSelf || target.discordUserId !== interaction.user.id,
  );

  if (eligibleTargets.length < targetSelection.count) {
    throw new Error(`Not enough eligible targets for variant ${variant.code}.`);
  }

  const options: RoleSetupTargetOption[] = eligibleTargets.map((target) => {
    const discordUser = interaction.client.users.cache.get(
      target.discordUserId,
    );

    const character = gameService.getCharacter(target.characterSlug);
    ...
    return {
      ...
      description:
        character === undefined ? target.characterSlug : character.name,
    };
  });

  await interaction.editReply({
    content: createRoleSetupTargetContent(setup, variant),
