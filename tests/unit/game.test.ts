...
import type {
  RoleAssignment,
} from "../../src/domain/roles/role-assignment.js";
...
describe("Game", () => {
  ...
  it("rejects duplicated role assignments", () => {
    ...
    expect(() => {
      ...
      "Role guardian was assigned more than once.",
    );
  });
  ...
  it("accepts valid secret assignments", () => {
    ...
    expect(
      game.secretAssignmentsPrepared,
    ).toBe(true);
  });
  ...
  it("can retrieve one player's secret assignments", () => {
    ...
    expect(
      game.getRoleAssignmentForPlayer(
        "alice",
      ),
    ).toEqual({
      playerId: "alice",
    ...
    expect(
      game.getObjectiveAssignmentsForPlayer(
        "alice",
      ),
    ).toHaveLength(2);
  });
