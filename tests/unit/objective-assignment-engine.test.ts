...
import { assignObjectives } from "../../src/application/objective-assignment/objective-assignment-engine.js";
...
function createObjective(
  ...
  return {
    ...
    rule: {
      ...
      increment: 1,
      requiredCount: type === "PRIMARY" ? 3 : 2,
    },
...
function createObjectives(): Objective[] {
  return [
    createObjective("primary-a", "PRIMARY"),

    createObjective("primary-b", "PRIMARY"),

    createObjective("primary-c", "PRIMARY"),

    createObjective("primary-d", "PRIMARY"),

    createObjective("secondary-a", "SECONDARY"),

    createObjective("secondary-b", "SECONDARY"),

    createObjective("secondary-c", "SECONDARY"),

    createObjective("secondary-d", "SECONDARY"),
  ];
}

const compatibilityRules: readonly ObjectiveCompatibilityRule[] = [
  {
  ...
  {
    leftTag: "REQUIRES_PLAYER_DEATH",

    rightTag: "FORBIDS_PLAYER_DEATH",
...
describe("ObjectiveAssignmentEngine", () => {
  it("assigns one primary and one secondary objective to every player", () => {
    const assignments = assignObjectives({
      seed: "objective-seed",
  ...
  it("produces a stable known assignment", () => {
    ...
    expect(
      assignments.map((assignment) => ({
        ...
        type: assignment.objectiveType,

        objective: assignment.objectiveCode,
      })),
    ).toEqual([
      {
        player: "alice",
  ...
  it("initializes progress from requiredCount", () => {
    ...
    const secondary = assignments.find(
      (assignment) => assignment.objectiveType === "SECONDARY",
    )!;

    expect(primary.progress).toEqual({
      current: 0,
      target: 3,
    });

    expect(secondary.progress).toEqual({
      current: 0,
      target: 2,
    });
  });
  ...
  it("ignores objectives unavailable for the player count", () => {
    ...
    const objectives = [
      createObjective("primary-valid", "PRIMARY", 2, 4),

      createObjective("primary-three-plus", "PRIMARY", 3, 4),

      createObjective("secondary-valid", "SECONDARY", 2, 4),
    ];
    ...
    expect(
      assignments.map((assignment) => assignment.objectiveCode),
    ).not.toContain("primary-three-plus");
  });

  it("fails when no primary objective is available", () => {
    const players = [createPlayer("alice"), createPlayer("bob")];

    const objectives = [createObjective("secondary", "SECONDARY")];
    ...
    }).toThrow("No primary objective is available for this game.");
  });

  it("fails when no secondary objective is available", () => {
    const players = [createPlayer("alice"), createPlayer("bob")];

    const objectives = [createObjective("primary", "PRIMARY")];
    ...
    }).toThrow("No secondary objective is available for this game.");
  });

  it("does not depend on input order", () => {
    const normal = assignObjectives({
      seed: "objective-seed",
    ...
    const reversed = assignObjectives({
      seed: "objective-seed",
  ...
  it("never assigns incompatible primary and secondary objectives to the same player", () => {
    ...
    const highGold: Objective = {
      ...createObjective("high-gold", "PRIMARY"),

      compatibilityTags: ["REQUIRES_HIGH_GOLD"],
    };

    const lowGold: Objective = {
      ...createObjective("low-gold", "SECONDARY"),

      compatibilityTags: ["REQUIRES_LOW_GOLD"],
    };

    const neutral: Objective = {
      ...createObjective("neutral", "SECONDARY"),

      compatibilityTags: ["INFORMATION"],
    };
    ...
    const assignments = assignObjectives({
      ...
      objectives: [highGold, lowGold, neutral],
    ...
    expect(secondaryAssignments).toHaveLength(2);

    expect(
      secondaryAssignments.every(
        (assignment) => assignment.objectiveCode === "neutral",
      ),
    ).toBe(true);
  });
  ...
  it("fails when no compatible secondary objective exists", () => {
    ...
    expect(() => {
      assignObjectives({
        seed: "impossible-combination",

        players,

        objectives: [highGold, lowGold],
  ...
  it("also applies compatibility rules based on other tags", () => {
    ...
    const requireDeath: Objective = {
      ...createObjective("require-death", "PRIMARY"),

      compatibilityTags: ["REQUIRES_PLAYER_DEATH"],
    };

    const forbidDeath: Objective = {
      ...createObjective("forbid-death", "SECONDARY"),

      compatibilityTags: ["FORBIDS_PLAYER_DEATH"],
    };

    const usePower: Objective = {
      ...createObjective("use-power", "SECONDARY"),

      compatibilityTags: ["POWER"],
    };

    const assignments = assignObjectives({
      seed: "death-compatibility",

      players,

      objectives: [requireDeath, forbidDeath, usePower],
    ...
    expect(
      assignments.some(
        (assignment) => assignment.objectiveCode === "forbid-death",
      ),
    ).toBe(false);

    expect(
      assignments.some(
        (assignment) => assignment.objectiveCode === "use-power",
      ),
    ).toBe(true);
  });
