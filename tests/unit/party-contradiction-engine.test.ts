...
function createObjective(code: string, tags: readonly string[]): Objective {
  return {
    code,
    name: code,
    description: code,
...
describe("PartyContradictionEngine", () => {
  it("returns zero when players have no conflicting tags", () => {
    const objectives = [
      createObjective("economy", ["ECONOMY"]),

      createObjective("information", ["INFORMATION"]),
    ];

    const assignments = [
      createAssignment("alice", "economy", "PRIMARY"),

      createAssignment("bob", "information", "PRIMARY"),
    ];

    const result = calculatePartyContradiction(assignments, objectives, rules);
  ...
  it("counts opposing death objectives between players", () => {
    const objectives = [
      createObjective("cause-death", ["REQUIRES_PLAYER_DEATH"]),

      createObjective("protect-all", ["FORBIDS_PLAYER_DEATH"]),
    ];

    const assignments = [
      createAssignment("alice", "cause-death", "PRIMARY"),

      createAssignment("bob", "protect-all", "PRIMARY"),
    ];
  ...
  it("counts a rule only once per player pair", () => {
    const objectives = [
      createObjective("sabotage-a", ["SABOTAGE"]),

      createObjective("sabotage-b", ["SABOTAGE"]),

      createObjective("protect", ["PROTECTIVE"]),
    ];

    const assignments = [
      createAssignment("alice", "sabotage-a", "PRIMARY"),

      createAssignment("alice", "sabotage-b", "SECONDARY"),

      createAssignment("bob", "protect", "PRIMARY"),
    ];
  ...
  it("adds different matching rules between the same players", () => {
    const objectives = [
      createObjective("corrupt", ["SABOTAGE", "REQUIRES_CURSE"]),

      createObjective("protect", ["PROTECTIVE"]),
    ];

    const assignments = [
      createAssignment("alice", "corrupt", "PRIMARY"),
  ...
  it("does not count contradictions inside the same player's objectives", () => {
    const objectives = [
      createObjective("sabotage", ["SABOTAGE"]),

      createObjective("protect", ["PROTECTIVE"]),
    ];

    const assignments = [
      createAssignment("alice", "sabotage", "PRIMARY"),

      createAssignment("alice", "protect", "SECONDARY"),
    ];
  ...
  it("fails when an assignment references an unknown objective", () => {
    expect(() => {
      calculatePartyContradiction(
        [createAssignment("alice", "unknown", "PRIMARY")],

        [],

        rules,
      );
    }).toThrow("Unknown objective code: unknown");
  });

  it("detects when a contradiction score is inside the configured budget", () => {
    expect(isContradictionWithinBudget(3, 4, budget)).toBe(true);
  });

  it("detects when a contradiction score is too low", () => {
    expect(isContradictionWithinBudget(0, 4, budget)).toBe(false);
  });

  it("detects when a contradiction score is too high", () => {
    expect(isContradictionWithinBudget(6, 4, budget)).toBe(false);
  });
});
