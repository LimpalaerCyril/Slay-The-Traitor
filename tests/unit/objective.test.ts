...
function createObjective(minimumPlayers = 2, maximumPlayers = 4): Objective {
  return {
    code: "test-objective",
    name: "Test Objective",
...
function createAssignment(target = 2): ObjectiveAssignment {
  return {
    playerId: "player-1",
...
describe("Objective", () => {
  it("is available for a valid player count", () => {
    ...
    expect(isObjectiveAvailableForPlayerCount(objective, 3)).toBe(true);
  });

  it("is unavailable below its minimum player count", () => {
    const objective = createObjective(3, 4);

    expect(isObjectiveAvailableForPlayerCount(objective, 2)).toBe(false);
  });

  it("is unavailable above its maximum player count", () => {
    const objective = createObjective(2, 3);

    expect(isObjectiveAvailableForPlayerCount(objective, 4)).toBe(false);
  });
  ...
  it("creates an assignment using the objective required count", () => {
    ...
    expect(assignment.progress).toEqual({
      current: 0,
      target: 3,
    });
  ...
  it("rejects an objective type not allowed by the definition", () => {
    ...
    expect(() => {
      createObjectiveAssignment("alice", objective, "PRIMARY");
    }).toThrow("Objective test-objective cannot be assigned as PRIMARY.");
  });
});
